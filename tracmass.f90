...
!== LOCAL VARIABLES ===
 ...
 INTEGER :: nnorth,ndrake,ngyre,nexit(NEND)

 REAL*8                                     :: x0, y0, z0, x1, y1, z1
 REAL*8                                     :: dse, dsw, dsn, dss, dsu, dsd
 REAL*8                                     :: ds, dsmin, dsc
 REAL*8                                     :: rlon,rlat
 REAL*8                                     :: tt, dt, t0, ts
 REAL*8                                     :: dxyz, subvol, dtreg
 REAL*8                                     :: ss0, rr, rb, rg, rbg, uu
 ...
 if(ds.eq.dse) then ! eastward grid-cell exit 
    ...
    x1=dble(ia)
      ...
      elseif(ds.eq.dsd) then ! downward grid-cell exit
         ! not needed         call vertvel(rb,ia,iam,ja,ka)

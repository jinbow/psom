...
!                                                                       
      ...
      double precision binu, fac,z,seval 
      double precision dep0(NK),dep1(NK),uvert0(NK),uvert1(NK),         &
     &     bu0(NK),cu0(NK),du0(NK),bu1(NK),cu1(NK),du1(NK),             &
     &     vvert0(NK),vvert1(NK),                                       &
     &     bv0(NK),cv0(NK),dv0(NK),bv1(NK),cv1(NK),dv1(NK)              
      double precision udif(NI,NJ,NK), vdif(NI,NJ,NK) 
      double precision ui(-1:NI+2),vi(-1:NI+2),                         &
     &     uxi(-1:NI+1),uxi2(0:NI+1),uxi3(0:NI),uxi4(1:NI),             &
     &     vxi(-1:NI+1),vxi2(0:NI+1),vxi3(0:NI),vxi4(1:NI),             &
     &     u4x(NI)                                                      
      double precision uj(-1:NJ+2),vj(-1:NJ+2),                         &
...
!                                                                       
         ...
         do k=1,NK 
            if (dabs(zc(0,j,k)-zc(1,j,k)).lt.1.d-16) then 
               ui(0)= u(1,j,k,m) 
               vi(0)= v(1,j,k,m) 
               ui(-1)= u(1,j,k,m) 
               vi(-1)= v(1,j,k,m) 
...
!                                                                       
            if (dabs(zc(NI+1,j,k)-zc(NI,j,k)).lt.1.d-16) then 
               ui(NI+1)= u(NI,j,k,m) 
               vi(NI+1)= v(NI,j,k,m) 
               ui(NI+2)= u(NI,j,k,m) 
               vi(NI+2)= v(NI,j,k,m) 
...
!                                                                       
         do k=1,NK 
            if (dabs(zc(i,1,k)-zc(i,0,k)).lt.1.d-16) then 
               uj(0)= u(i,1,k,m) 
               vj(0)= v(i,1,k,m) 
               uj(-1)= u(i,1,k,m) 
               vj(-1)= v(i,1,k,m) 
...
!                                                                       
            if (dabs(zc(i,NJ,k)-zc(i,NJ+1,k)).lt.1.d-16) then 
               uj(NJ+1)= u(i,NJ,k,m) 
               vj(NJ+1)= v(i,NJ,k,m) 
               uj(NJ+2)= u(i,NJ,k,m) 
               vj(NJ+2)= v(i,NJ,k,m) 

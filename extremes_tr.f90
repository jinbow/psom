subroutine extremes(step) 
  ...
  !     Modified for tracer                                               
      ...
      integer i,j,k,it,step 
      double precision umax,vmax,wmax,Trmax(ntr),smax,Tmax,umin,vmin,wmin,  &
          smin,Tmin,Trmin(ntr)                                               
      double precision wup(NK),wdn(NK),const 
...
!     FIND wup and wdn: the average vertical velocity in up and dn direc
      ...
      do k=NK,1,-1 
         ...
         do j=1,NJ 
            do i=1,NI 
               wup(k) = wup(k) + 0.5*(w(i,j,k,0) +dabs(w(i,j,k,0))) 
               wdn(k) = wdn(k) + 0.5*(dabs(w(i,j,k,0))-w(i,j,k,0)) 
            end do 
         end do 
         wup(k) = wup(k)*const 
         wdn(k) = wdn(k)*const 
